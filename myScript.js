...
window.onload = () => {
  navTimeout = setTimeout(hideNavbar, 5000);
};
...
function scrollToServices() {
  const section = document.getElementById("servicesTitle");
...
function checkReminders() {
    ...
    reminders.forEach(reminder => {
        ...
        if(
            ...
            showCustomNotification(reminder.text);
...
function addRecord() {
    ...
    records.push({ type, value, unit, date, notes, image: imageData });
...
function displayRecords() {
    ...
    records.forEach((record, index) => {
        ...
        <td>${record.notes}</td>
        <td>${record.image ? `<img src="${record.image}" width="50">` : "-"}</td>
        <td><button onclick="deleteRecord(${index})">Delete</button></td>
        <td><button src="${record.image}" width="50" onclick="window.open('${record.image}')">View</td>
        `;
...
function addMedicine() {
    ...
    const total = document.getElementById("medTotal").value;

    if (!name || !dosage || !time) return alert("Enter medicine, dosage, and time");

    const meds = JSON.parse(localStorage.getItem("medicines")) || [];
    meds.push({ name, dosage, time, total: parseInt(total), remaining: parseInt(total)});
...
function displayMedicines() {
    ...
    if (meds.length === 0) {
        list.innerHTML = `<tr><td colspan="5">No medicines added yet</td></tr>`;
        return;
    }
    ...
    meds.forEach((med, index) => {
        ...
        row.innerHTML = `
            ...
            <td style="color:${med.remaining <= 2 ? 'red' : 'green'}; font-weight:bold">${med.remaining}</td>
            <td>
...
function markTaken(index) {
    ...
    if(meds[index].remaining > 0){

        meds[index].remaining--;

        alert("Medicine taken!");

        if(meds[index].remaining <= 2){
            alert("Only a few medicines left!");
        }

    } else {

        alert("No medicines left!");
